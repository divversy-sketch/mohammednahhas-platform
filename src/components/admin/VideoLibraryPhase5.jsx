...
const getProvider = (item = {}) => {
  const url = String(item.url || item.file || item.videoUrl || "");
...
export default function VideoLibraryPhase5({ content = [], onOpenContent, onGoContentManager }) {
  ...
  const grades = useMemo(() => {
    return ["all", ...Array.from(new Set(videos.map(v => v.grade).filter(Boolean)))];
  }, [videos]);
  ...
  const filtered = useMemo(() => {
    ...
    return videos.filter((v) => {
      ...
      const byGrade = grade === "all" || v.grade === grade;
      const byProvider = provider === "all" || p === provider;
      return byText && byGrade && byProvider;
    });
      ...
      <div className="rounded-[2rem] bg-gradient-to-l from-slate-950 via-red-950 to-orange-700 text-white p-6 md:p-8 shadow-2xl">
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-5">
          <div>
            ...
            <h2 className="text-3xl md:text-4xl font-black mb-2">صفحة فيديوهات منظمة</h2>
            <p className="text-orange-100 max-w-3xl">
              كل الفيديوهات في مكان واحد بشكل منظم حسب الصف والفرع، مع دعم YouTube الأساسي وVimeo كاحتياطي.
            </p>
          </div>

          <button
            onClick={onGoContentManager}
            className="bg-white text-slate-950 px-5 py-3 rounded-2xl font-black flex items-center justify-center gap-2 shadow-xl"
          >
            <PlusCircle size={18} /> إضافة/إدارة المحتوى
          </button>
        </div>
      </div>
          ...
          <div className="flex flex-col lg:flex-row gap-3 w-full 2xl:w-auto">
            <div className="relative flex-1 lg:w-96">
              ...
              <input
                ...
                placeholder="بحث باسم الفيديو أو الفرع..."
                className="w-full pr-11 pl-4 py-3 rounded-2xl border bg-slate-50 outline-none focus:border-orange-500"
              />
            </div>
          ...
          <div className="space-y-6">
            {Object.entries(groups).map(([g, branches]) => (
              <div key={g} className="border rounded-3xl overflow-hidden">
                <div className="bg-slate-950 text-white p-4 font-black text-lg">
                  {getGradeLabel(g)}
                </div>
                  ...
                  {Object.entries(branches).map(([branch, items]) => (
                    <div key={branch}>
                      ...
                      <div className="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-4">
                        {items.map((item) => (
                          <VideoCard key={item.id} item={item} onOpen={() => onOpenContent?.(item)} />
                        ))}
                      </div>
                    </div>
                  ))}
...
function VideoCard({ item, onOpen }) {
  ...
  const providerStyle = provider === "youtube"
  ...
  return (
    <div className="bg-white border rounded-3xl p-5 shadow-sm hover:shadow-xl hover:-translate-y-1 transition">
      <div className="flex items-start justify-between gap-3 mb-4">
        ...
        <span className={`px-3 py-1 rounded-full text-xs font-black border ${providerStyle}`}>
          {provider === "youtube" ? "YouTube" : provider === "vimeo" ? "Vimeo احتياطي" : "مباشر"}
        </span>
      </div>

      <h5 className="text-lg font-black text-slate-950 mb-2 line-clamp-2">{item.title || "فيديو بدون عنوان"}</h5>
      <p className="text-sm text-slate-500 mb-4">
        {item.unit || item.chapter || "بدون وحدة"} • {item.branch || "عام"}
      </p>

      <div className="flex items-center gap-2 text-xs text-slate-500 mb-4">
        <Clock size={14} />
        <span>{item.estimatedDurationMinutes ? `${item.estimatedDurationMinutes} دقيقة تقريبًا` : "مدة غير محددة"}</span>
      </div>

      <div className="flex gap-2">
        <button onClick={onOpen} className="flex-1 bg-slate-950 text-white py-3 rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-slate-800 transition">
          <PlayCircle size={17} /> تشغيل
        </button>
        {(item.url || item.file) && (
          <a href={item.url || item.file} target="_blank" rel="noopener noreferrer" className="bg-slate-100 text-slate-700 p-3 rounded-2xl hover:bg-slate-200 transition">
