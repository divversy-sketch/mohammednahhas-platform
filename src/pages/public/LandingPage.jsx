          ...
          <div className="bg-white/80 backdrop-blur p-4 md:p-6 rounded-3xl border border-white shadow-sm overflow-hidden">
            ...
            <div className="space-y-4">
              {videos.length > 0 ? videos.map((v, i) => (
               ...
               )) : <p className="text-slate-500 text-sm">{loading ? 'جاري تحميل الفيديوهات...' : error ? 'تعذر تحميل الفيديوهات حالياً' : 'لا توجد فيديوهات عامة حالياً'}</p>}
            </div>
          </div>
          <div className="bg-white/80 backdrop-blur p-4 md:p-6 rounded-3xl border border-white shadow-sm overflow-hidden">
            <h3 className="text-xl md:text-2xl font-bold mb-4 flex items-center gap-2 text-purple-700"><Code /> تفاعلي للجميع</h3>
            <div className="space-y-4">
              {htmls.length > 0 ? htmls.map((h, i) => (
               ...
               )) : <p className="text-slate-500 text-sm">{loading ? 'جاري تحميل المحتوى...' : error ? 'تعذر تحميل المحتوى حالياً' : 'لا يوجد محتوى تفاعلي عام حالياً'}</p>}
            </div>
          </div>
        </div>
      </main>
