...
/**
 * =================================================================
 * 3. المكونات الرسومية والستايلات (تحديث العلامة المائية)
 * =================================================================
 */
...
const DesignSystemLoader = () => {
  ...
  return (
    <style>{`
      ...
      /* --- العلامة المائية المحدثة والمتحركة --- */
      .watermark-container {
        position: absolute;
        inset: 0;
        pointer-events: none;
      ...
      @keyframes floatWatermark {
        ...
        25% { transform: translate(35vw, 20vh) rotate(-25deg); }
        50% { transform: translate(65vw, -10vh) rotate(-35deg); }
        75% { transform: translate(25vw, 45vh) rotate(-30deg); }
        100% { transform: translate(0, 0) rotate(-30deg); }
      }

      .watermark-moving {
        animation: floatWatermark 28s ease-in-out infinite alternate;
      }
...
// مكون العلامة المائية المتحركة (ثابت المواقع لمنع التذبذب)
const DynamicWatermark = ({ text, type = 'exam' }) => {
    const positions = useMemo(() => [
        { top: '15%', left: '15%', delay: '0s' },
        { top: '45%', left: '75%', delay: '-7s' },
        { top: '85%', left: '25%', delay: '-14s' },
        { top: '25%', left: '55%', delay: '-21s' }
    ], []);
    ...
    return (
        <div className="watermark-container">
            {positions.map((pos, i) => (
                <div 
                    key={i} 
                    className={`watermark-item watermark-moving ${type === 'video' ? 'watermark-video' : 'watermark-exam'}`}
                    style={{ 
                        top: pos.top, 
                        left: pos.left,
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900 flex flex-col md:flex-row font-['Cairo']" dir="rtl">
      <div className="flex-1 flex flex-col relative overflow-hidden">
        ...
        <div className="bg-amber-600 p-3 text-white flex justify-between items-center shadow-lg relative z-10">
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/95 flex items-center justify-center p-4">
      <div className="w-full max-w-5xl bg-black rounded-xl overflow-hidden relative shadow-2xl border border-gray-800">
        <div className="absolute top-4 right-4 z-[70] flex gap-4">
        ...
        <div className="aspect-video relative flex items-center justify-center bg-black overflow-hidden">
          {/* العلامة المائية هنا */}
          <DynamicWatermark text={`${userName} - ${video.grade}`} type="video" />
          
          {videoId ? (
            <iframe className="w-full h-full relative z-10" src={youtubeEmbedUrl} title="Video" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen></iframe>
          ) : (
             <video ref={videoRef} controls controlsList="nodownload" className="w-full h-full object-contain relative z-10" src={finalUrl}>المتصفح لا يدعم هذا الفيديو.</video>
          )}
        </div>
      </div>
    </div>
...
useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-100 flex flex-col font-['Cairo'] no-select" dir="rtl">
      {/* العلامة المائية المتحركة للامتحان */}
            ...
            <div className="space-y-4">
              {currentQObj.options.map((opt, idx) => {
                  ...
                  if (isReviewMode) {
                      ...
                      if (answers[currentQObj.id] === idx) optionClass = 'border-amber-500 bg-amber-50 text-amber-900 shadow-sm scale-[1.02]';
                  }
                  ...
                  return (
                    <div key={idx} onClick={() => handleAnswer(currentQObj.id, idx)} className={`p-5 rounded-xl border-2 cursor-pointer transition-all flex items-center gap-4 text-lg font-medium ${optionClass}`}>
                      <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${answers[currentQObj.id] === idx || (isReviewMode && idx === currentQObj.correctIdx) ? 'border-transparent bg-amber-600' : 'border-slate-300'}`}>
                        {(answers[currentQObj.id] === idx || (isReviewMode && idx === currentQObj.correctIdx)) && <div className="w-2 h-2 bg-white rounded-full" />}
                      </div>
...
// --- 4. لوحة تحكم الأدمن (النسخة الكاملة بدون اختصار) ---
    ...
    <div className="min-h-screen bg-slate-100 p-6 font-['Cairo']" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="md:col-span-3">
                  ...
                  <form onSubmit={handleAddContent} className="grid gap-4 mb-6">
                      ...
                      <div className="border p-3 rounded-lg bg-gray-50">
                          ...
                          <input 
                            ...
                            placeholder="اكتب إيميلات الطلاب مفصولة بفاصلة" 
          ...
          {activeTab === 'messages' && <div className="bg-white p-6 rounded-xl shadow-sm"><h2 className="font-bold mb-4">الرسائل</h2>{messagesList.map(m=><div key={m.id} className="border-b p-4 bg-slate-50 mb-3 rounded-lg relative"><button onClick={()=>handleDeleteMessage(m.id)} className="absolute top-2 left-2 text-red-400"><Trash2 size={16}/></button><div className="mb-2"><p className="font-bold text-amber-800">{m.senderName} <span className="text-xs text-slate-500">({m.sender})</span></p></div><p className="text-slate-800 bg-white p-3 rounded-lg border border-slate-200 mb-3">{m.text}</p>{m.adminReply?<div className="bg-green-50 p-3 rounded-lg border border-green-200 text-sm"><span className="font-bold text-green-700">ردك: </span>{m.adminReply}</div>:<div className="flex gap-2"><input className="flex-1 border p-2 rounded text-sm" placeholder="اكتب ردك..." value={replyTexts[m.id]||""} onChange={e=>setReplyTexts({...replyTexts,[m.id]:e.target.value})}/><button onClick={()=>handleReplyMessage(m.id)} className="bg-blue-600 text-white px-3 py-1 rounded text-sm"><Reply size={14}/></button></div>}</div>)}</div>}
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm">
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl border mb-6">
                      ...
                      <div className="grid gap-3">
                          <input className="border p-2 rounded" placeholder="الكلمات المفتاحية (مثال: سعر,حجز)" value={newAutoReply.keywords} onChange={e=>setNewAutoReply({...newAutoReply, keywords:e.target.value})} />
                          <textarea className="border p-2 rounded h-20" placeholder="الرد..." value={newAutoReply.response} onChange={e=>setNewAutoReply({...newAutoReply, response:e.target.value})} />
                          <button onClick={handleAddAutoReply} className="bg-amber-600 text-white py-2 rounded font-bold hover:bg-amber-700">إضافة</button>
                      </div>
                  </div>
                  <div className="space-y-3">
                      {autoReplies.map(rule => (
                          <div key={rule.id} className={`p-4 rounded-lg border flex justify-between items-center ${rule.isActive ? 'bg-white border-green-200' : 'bg-gray-50 border-gray-200'}`}>
                              <div className="flex-1">
                                  <p className="font-bold text-sm">الكلمات: {rule.keywords}</p>
                                  <p className="text-slate-800">{rule.response}</p>
                              </div>
                              <div className="flex items-center gap-2">
                                  <button onClick={() => toggleAutoReply(rule.id, rule.isActive)} className={`p-2 rounded-full ${rule.isActive ? 'bg-green-100 text-green-600' : 'bg-gray-200'}`}><Power size={18} /></button>
                                  <button onClick={() => deleteAutoReply(rule.id)} className="p-2 rounded-full bg-red-100 text-red-600"><Trash2 size={18} /></button>
                              </div>
                          </div>
                      ))}
                  </div>
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm">
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl border mb-6">
                      <div className="grid gap-3">
                          ...
                          <input className="border p-2 rounded" placeholder="المصدر" value={newQuote.source} onChange={e=>setNewQuote({...newQuote, source:e.target.value})} />
                          <button onClick={handleAddQuote} className="bg-amber-600 text-white py-2 rounded font-bold">إضافة</button>
                      </div>
                  </div>
                  <div className="space-y-3">
                      {quotesList.map(q => (
                          <div key={q.id} className="p-3 rounded-lg border bg-white flex justify-between items-center">
                              <div><p className="font-bold">"{q.text}"</p><p className="text-xs">- {q.source}</p></div>
                              <button onClick={() => deleteQuote(q.id)} className="p-2 text-red-500"><Trash2 size={18} /></button>
                          </div>
                      ))}
                  </div>
              </div>
          ...
          {activeTab === 'settings' && (
              <div className="bg-white p-6 rounded-xl shadow-sm space-y-6">
                      ...
                      <div className="space-y-1">
                          {announcements.map(a => (
                              <div key={a.id} className="flex justify-between items-center bg-slate-50 p-2 rounded">
                                  ...
                                  <button onClick={() => handleDeleteAnnouncement(a.id)} className="text-red-500"><Trash2 size={14}/></button>
                              </div>
                          ))}
                      </div>
                  </div>
                  <div className="border p-4 rounded-xl flex justify-between items-center">
                      ...
                      <button onClick={toggleLeaderboard} className={`px-6 py-2 rounded-full font-bold ${showLeaderboard ? 'bg-green-100 text-green-700' : 'bg-slate-200'}`}>
...
const StudentDashboard = ({ user, userData }) => {
  ...
  useEffect(() => {
    ...
    onSnapshot(query(collection(db, 'live_sessions'), where('status', '==', 'active'), where('grade', '==', userData.grade)), s => setLiveSession(s.empty ? null : {id:s.docs[0].id, ...s.docs[0].data()}));
    onSnapshot(query(collection(db, 'exams'), where('grade', '==', userData.grade)), s => setExams(s.docs.map(d=>({id:d.id,...d.data()}))));
    onSnapshot(query(collection(db, 'exam_results'), where('studentId', '==', user.uid)), s => setExamResults(s.docs.map(d=>({id:d.id,...d.data()}))));
    
    onSnapshot(query(collection(db, 'notifications'), where('grade', 'in', ['all', userData.grade]), orderBy('createdAt', 'desc'), limit(10)), s => {
        const newNotifs = s.docs.map(d => d.data());
        setNotifications(newNotifs);
        if(newNotifs.length > 0) setHasNewNotif(true);
    });
  ...
  const startExamWithCode = async (exam) => {
    ...
    if (previousResult) return alert("لقد قمت بفتح هذا الامتحان سابقاً.");
    ...
    if (code === exam.accessCode) {
        const attemptRef = await addDoc(collection(db, 'exam_results'), { 
            examId: exam.id, studentId: user.uid, studentName: user.displayName, score: 0, status: 'started', startedAt: serverTimestamp() 
        });
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 relative font-['Cairo']" dir="rtl">
      ...
      <main className="flex-1 p-4 md:p-10 relative z-10 overflow-y-auto h-screen">
        <div className="md:hidden flex justify-between items-center mb-6 bg-white/80 p-4 rounded-2xl shadow-sm"><h1 className="font-bold text-lg">منصة النحاس</h1><button onClick={() => setMobileMenu(true)} className="p-2 bg-slate-100 rounded-lg"><Menu /></button></div>
        
        <div className="flex justify-end mb-6 relative">
            <button onClick={() => setShowNotifications(!showNotifications)} className="relative p-2 bg-white rounded-full shadow-sm">
            ...
            {showNotifications && (
                <div className="absolute top-12 left-0 w-80 bg-white rounded-xl shadow-xl border p-4 z-50">
                    <h3 className="font-bold mb-3 text-sm">الإشعارات</h3>
                    <div className="space-y-2">
                        {notifications.map((n, i) => (
                            <div key={i} className="text-sm bg-slate-50 p-2 rounded border-l-4 border-amber-500">{n.text}</div>
                        ))}
                    </div>
                </div>
            )}
        ...
        {activeTab === 'home' && (<div className="space-y-8"><WisdomBox /><Announcements /><h2 className="text-3xl font-bold">منور يا {userData.name.split(' ')[0]} 👋</h2><div className="grid grid-cols-1 md:grid-cols-3 gap-6"><div onClick={()=>setActiveTab('videos')} className="bg-blue-600 text-white p-8 rounded-3xl cursor-pointer hover:scale-105 transition"><h3 className="text-2xl font-bold mb-2">المحاضرات</h3><p className="text-4xl font-black">{videos.length}</p></div><div onClick={()=>setActiveTab('files')} className="bg-amber-500 text-white p-8 rounded-3xl cursor-pointer hover:scale-105 transition"><h3 className="text-2xl font-bold mb-2">الملفات</h3><p className="text-4xl font-black">{files.length}</p></div><div onClick={()=>setActiveTab('exams')} className="bg-slate-800 text-white p-8 rounded-3xl cursor-pointer hover:scale-105 transition"><h3 className="text-2xl font-bold mb-2">الامتحانات</h3><p className="text-4xl font-black">{exams.length}</p></div></div><Leaderboard /></div>)}
        {activeTab === 'videos' && <div className="grid grid-cols-1 md:grid-cols-3 gap-6">{videos.map(v => (<div key={v.id} className="bg-white rounded-xl shadow-sm border overflow-hidden cursor-pointer hover:shadow-md transition" onClick={() => setPlayingVideo(v)}><div className="h-40 bg-slate-800 flex items-center justify-center"><PlayCircle className="text-white w-12 h-12 opacity-80"/></div><div className="p-4"><h3 className="font-bold">{v.title}</h3></div></div>))}</div>}
        {activeTab === 'files' && <div className="bg-white rounded-xl shadow-sm border overflow-hidden">{files.map(f => (<div key={f.id} className="p-4 flex justify-between items-center border-b last:border-0"><div className="flex items-center gap-4"><div className="bg-red-100 text-red-600 p-3 rounded-lg font-bold">PDF</div><div><h4 className="font-bold text-lg">{f.title}</h4></div></div><a href={f.url} target="_blank" className="bg-blue-50 text-blue-600 px-4 py-2 rounded-lg font-bold">تحميل</a></div>))}</div>}
        
        {activeTab === 'exams' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {isBanned ? (
                <div className="col-span-full bg-red-50 border p-8 rounded-3xl text-center">
                    <AlertTriangle size={48} className="text-red-500 mx-auto mb-4" />
                    <h3 className="text-2xl font-bold text-red-800">تم حظرك من الامتحانات</h3>
                </div>
            ) : exams.map(e => {
                const prevResult = examResults.find(r => r.examId === e.id);
                return (
                  <div key={e.id} className="bg-white p-6 rounded-2xl shadow-sm border relative">
                    {prevResult && <div className="absolute top-0 left-0 bg-green-500 text-white text-xs px-3 py-1 rounded-br-xl font-bold">الدرجة: {prevResult.score}</div>}
                    <h3 className="text-xl font-bold mb-2">{e.title}</h3>
                    <div className="flex justify-between text-sm text-slate-500 mb-4"><span>⏳ {e.duration} دقيقة</span></div>
                    {prevResult ? (
                        <div className="flex gap-2">
                        ...
                        <button onClick={() => startExamWithCode(e)} className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2"><Lock size={16}/> ابدأ الامتحان</button>
                    )}
                  </div>
                )
            })}
        ...
        {activeTab === 'settings' && (
             <div className="bg-white p-8 rounded-xl shadow-sm max-w-2xl mx-auto">
               <h2 className="text-2xl font-bold mb-6 flex items-center gap-2"><Settings/> إعدادات الحساب</h2>
               <form onSubmit={handleUpdateMyProfile} className="space-y-4">
                 ...
                 <div><label className="block text-sm font-bold text-slate-700 mb-2">الصف الدراسي</label><select className="w-full border p-3 rounded-xl" value={editFormData.grade} onChange={e=>setEditFormData({...editFormData, grade:e.target.value})}><GradeOptions /></select></div>
                 <button className="w-full bg-amber-600 text-white py-3 rounded-xl font-bold">حفظ التعديلات</button>
               </form>
             </div>
        )}
      </main>
...
// --- 6. الصفحة الرئيسية العامة (Landing) ---
const LandingPage = ({ onAuthClick }) => {
  const [publicContent, setPublicContent] = useState([]);
  const [playingVideo, setPlayingVideo] = useState(null); 
  
  useEffect(() => { onSnapshot(query(collection(db, 'content'), where('isPublic', '==', true)), s => setPublicContent(s.docs.map(d=>d.data()))); }, []);
  const openFacebook = () => window.open("https://www.facebook.com/share/17aiUQWKf5/", "_blank");
    ...
    <div className="min-h-screen font-['Cairo'] relative" dir="rtl">
      ...
      <main className="relative z-10 px-4 mt-10 max-w-7xl mx-auto text-center">
        ...
        <div className="grid md:grid-cols-2 gap-8 mt-10 mb-20">
          <div className="bg-white/80 backdrop-blur p-6 rounded-3xl border shadow-sm">
            <h3 className="text-2xl font-bold mb-4 flex items-center gap-2 text-blue-700"><Video /> فيديوهات مجانية</h3>
            <div className="space-y-4">
              {publicContent.filter(c => c.type === 'video').map((v, i) => (
                 <div key={i} className="flex items-center gap-3 p-3 bg-white rounded-xl shadow-sm cursor-pointer hover:bg-gray-50" onClick={() => setPlayingVideo(v)}>
                    <PlayCircle className="text-amber-500"/><span className="font-bold">{v.title}</span><span className="mr-auto text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded">مشاهدة</span>
                 </div>
              ))}
            </div>
          </div>
          <div className="bg-white/80 backdrop-blur p-6 rounded-3xl border shadow-sm">
            ...
            <div className="space-y-4">
              {publicContent.filter(c => c.type === 'file').map((f, i) => (
...
const AuthPage = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-900 font-['Cairo'] relative overflow-hidden" dir="rtl">
      <FloatingArabicBackground />
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl relative z-10">
        <button onClick={onBack} className="text-slate-500 text-sm mb-6 flex items-center gap-1"><ChevronRight size={18} /> العودة</button>
        <h2 className="text-3xl font-black mb-6 text-center">{isRegister ? 'حساب جديد' : 'تسجيل دخول'}</h2>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          {isRegister && (
            <>
              <input required className="w-full py-3 px-4 rounded-xl border bg-slate-50" placeholder="الاسم ثلاثي" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
              <input required className="w-full py-3 px-4 rounded-xl border bg-slate-50" placeholder="رقم هاتفك" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
              <input required className="w-full py-3 px-4 rounded-xl border bg-slate-50" placeholder="رقم ولي الأمر" value={formData.parentPhone} onChange={e => setFormData({...formData, parentPhone: e.target.value})} />
              <select className="w-full py-3 px-4 rounded-xl border bg-slate-50" value={formData.grade} onChange={e => setFormData({...formData, grade: e.target.value})}><GradeOptions /></select>
            </>
          )}
          <input required type="email" className="w-full py-3 px-4 rounded-xl border bg-slate-50" placeholder="البريد" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
          <input required type="password" className="w-full py-3 px-4 rounded-xl border bg-slate-50" placeholder="كلمة السر" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} />
          <button disabled={loading} className="bg-amber-600 text-white py-3 rounded-xl font-bold">{loading ? <Loader2 className="animate-spin mx-auto" /> : (isRegister ? 'تسجيل' : 'دخول')}</button>
        </form>
        <button onClick={() => setIsRegister(!isRegister)} className="mt-6 text-amber-700 font-bold w-full text-center block">{isRegister ? 'تسجيل الدخول' : 'حساب جديد'}</button>
      </motion.div>
    </div>
  );
};
...
export default function App() {
  ...
  if (loading) return <div className="h-screen flex items-center justify-center"><Loader2 className="animate-spin text-amber-600 w-12 h-12"/></div>;

  return (
    <AnimatePresence mode='wait'>
      <DesignSystemLoader />
      {!user ? (
        viewMode === 'landing' ? <LandingPage onAuthClick={() => setViewMode('auth')} /> : <AuthPage onBack={() => setViewMode('landing')} />
      ) : (
        user.email === 'mido16280@gmail.com' ? <AdminDashboard user={user} /> : <StudentDashboard user={user} userData={userData} />
      )}
    </AnimatePresence>
  );
}